    ...
    $.ajax({
      ...
      success: function (data) {
        ...
        if (data.data == true) {
          ...
          $("#modal-message")
            .find(".modal-body")
            .text("Student Save Successfully!");
...
$(".confirm-party").click(function (event) {
  ...
  $.ajax({
    ...
    success: function (data) {
      ...
      $("#modal-message")
        .find(".modal-body")
        .text("Party Remove Successfully!");
      showAllParty();
    },
    error: function () {
      alert("Error: L386+");
...
function getStud(id) {
  $.ajax({
    ...
    success: function (data) {
      $("#modal-run")
        .find(".modal-title")
        .text(data.stud_fN + " " + data.stud_mN + " " + data.stud_lN);
    },
    error: function () {
      alert("Error: ");
    },
...
$(document).on("submit", "#form-run", function (event) {
  ...
  var validate = "";
  var form_data = new Array($("select[id=pos]"), $("select[id=party]"));
...
$("#end-election").click(function (event) {
  ...
  $("#modal-confirmation")
    .find(".modal-title")
    .text("Are you sure you want to End the Election?");
  $("#modal-confirmation").modal("show");
});
